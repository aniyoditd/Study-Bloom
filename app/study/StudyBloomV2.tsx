...
import { CloudSyncIndicator, useCloudSync } from "./CloudSync";
...
function migrateState(): StudyState {
  try {
    ...
    if (saved) {
      ...
      const profile = { ...DEFAULT_STATE.profile, ...parsed.profile };
      if (parsed.profile?.xpVersion !== 2) { profile.xp = 0; profile.xpVersion = 2; }
      return { ...DEFAULT_STATE, ...parsed, profile, classes: (parsed.classes || DEFAULT_STATE.classes).map((item) => ({ ...item, subject: item.subject || "Other" })), tasks: (parsed.tasks || DEFAULT_STATE.tasks).map((task) => ({ ...task, category: task.category || "Homework", priority: task.priority || "Normal", status: task.done ? "Submitted" : task.status || "To do", progress: task.done ? 100 : task.progress || 0, timeSpent: task.timeSpent || 0, sessions: task.sessions || 0, xpAwarded: task.xpAwarded || false })), version: 3 };
    }
    const old = localStorage.getItem("studybloom-v1");
    if (!old) return DEFAULT_STATE;
    const legacy = JSON.parse(old);
    return { ...DEFAULT_STATE, profile: { ...DEFAULT_STATE.profile, name: legacy.name || DEFAULT_STATE.profile.name } };
...
export default function StudyBloomV2() {
  ...
  const cloud = useCloudSync(state, setState, loaded);
  ...
  function toggleTask(id: string) { const task = state.tasks.find((item) => item.id === id); const completing = Boolean(task && !task.done); setState((current) => ({ ...current, tasks: current.tasks.map((item) => item.id === id ? { ...item, done: !item.done, status: !item.done ? "Submitted" : "To do", progress: !item.done ? 100 : Math.min(item.progress || 0, 95), xpAwarded: completing ? true : item.xpAwarded } : item) })); if (task && completing && !task.xpAwarded) addActivity(task.classId, "task", task.minutes, `Completed: ${task.title}`); }
  function setTaskStatus(id: string, status: NonNullable<StudyTask["status"]>) { const task = state.tasks.find((item) => item.id === id); const firstCompletion = status === "Submitted" && task && !task.done && !task.xpAwarded; setState((current) => ({ ...current, tasks: current.tasks.map((item) => item.id === id ? { ...item, status, done: status === "Submitted", progress: status === "Submitted" ? 100 : item.progress || 0, xpAwarded: firstCompletion ? true : item.xpAwarded } : item) })); if (firstCompletion) addActivity(task.classId, "task", task.minutes, `Completed: ${task.title}`); }
  function updateTaskProgress(id: string, progress: number) { const task = state.tasks.find((item) => item.id === id); const firstCompletion = progress >= 100 && task && !task.done && !task.xpAwarded; setState((current) => ({ ...current, tasks: current.tasks.map((item) => item.id === id ? { ...item, progress, status: progress >= 100 ? "Submitted" : progress > 0 ? "In progress" : "To do", done: progress >= 100, xpAwarded: firstCompletion ? true : item.xpAwarded } : item) })); if (firstCompletion) addActivity(task.classId, "task", task.minutes, `Completed: ${task.title}`); }
  function setXp(value: number) { const xp = Math.max(0, Math.min(1000000, Math.round(value))); setState((current) => ({ ...current, profile: { ...current.profile, xp, xpVersion: 2 } })); setToast(`XP changed to ${xp.toLocaleString()}.`); }
  ...
  return <main className="sb2-shell" style={{ "--sb-purple": state.profile.accent } as React.CSSProperties}>
    ...
    <ToolDock/>{xpOpen && <XpPanel state={state} close={() => setXpOpen(false)} setXp={setXp}/>} {installOpen && <InstallPanel close={() => setInstallOpen(false)} nativeInstall={installApp} canNativeInstall={Boolean(installPrompt)}/>} {courseEditorId && state.classes.find((course) => course.id === courseEditorId) && <CourseEditor course={state.classes.find((course) => course.id === courseEditorId)!} close={() => setCourseEditorId(null)} save={updateCourse}/>} 
